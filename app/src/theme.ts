...
// Тёмная тема собирается целиком, а не наследует светлую: в ней другие задачи.
//
// Светлая различает карточку и фон тенью — в тёмной теней нет, и вся нагрузка
// падает на разницу яркостей и границу. Поэтому фон уводится глубже (#0F0F12),
// поверхность поднимается (#202028), а граница становится заметной: было
// 1.09 между фоном и карточкой, стало 1.18, между границей и карточкой —
// с 1.23 до 1.44.
//
// Основной текст, наоборот, приглушается: 15.5:1 — это больше, чем нужно для
// чтения, и именно от такого контраста устают глаза. 13:1 остаётся сильно выше
// порога AAA (7:1).
//
// Статусные цвета в светлой теме рассчитаны на белый фон; на тёмном насыщенный
// #EF4444 вибрирует, поэтому берутся более светлые и менее насыщенные тона.
const dark: Colors = {
  bg: '#0F0F12',
  surface: '#202028',
  border: '#3A3A45',
  ...
  accentHover: '#A3A5FF',
  accentActive: '#6F72E8',
  accentSoft: 'rgba(139,141,245,0.18)',
  success: '#34D399',
  warning: '#FBBF24',
  error: '#F87171',
};
