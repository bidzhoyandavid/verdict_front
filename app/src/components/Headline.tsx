...
export function Headline({ results }: { results: TestResults }) {
  ...
  const badge = t.verdictCodes[verdict.code] ?? verdict.label.toUpperCase();
  ...
  const direction = diff === null ? '' : diff >= 0 ? t.higher : t.lower;
    ...
    >
      ...
      {verdict.metric && (
        <div style={{ padding: '12px 18px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={caption}>{t.whatWasComputed}</div>
          <div style={{ fontSize: 15, color: c.textPrimary, lineHeight: 1.45 }}>
            ...
            {pair && (
              <>
                ...
                {diff !== null ? t.againstControlWithDiff(direction) : t.againstControl}
      ...
      <div style={{ padding: '12px 18px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={caption}>{t.howPrecise}</div>
        <div style={{ fontSize: 13, color: c.textSecondary, lineHeight: 1.5 }}>
          ...
          {verdict.significant === false && t.notSignificant}
        </div>
      </div>
      ...
      <div style={{ padding: '12px 18px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={caption}>{t.whatToDo}</div>
        ...
        {verdict.caveats.length > 0 && (
          <div style={{ fontSize: 12, color: c.warning }}>
            {t.caveatsBelow(verdict.caveats.length)}
          </div>
        )}
